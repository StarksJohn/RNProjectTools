...
export function isNull (str) {
  return (!str || str.length === 0 || str === '' || str.replace(/(^s*)|(s*$)/g, '').length === 0 || new RegExp('^[ ]+$').test(str) || typeof str === 'null' || typeof str === 'undefined')
}
...
export function cutLinkingUrl () {
  ...
  let host = null, params = {}, path = null
  ...
  return {
    host,/*唤醒哪个app*/ path/*页面*/, params/*参数*/
  }
}
...
export function parseUrl (url) {
  ...
  let scheme = 'http: || https:', host = 'xxx.xxx.com', port = ':xxxx', path = '/xxx/xxx', params = '?xxx=xxx&xxx=xxx'
  ...
  return {
    host,/*唤醒哪个app*/ path/*页面*/, params/*返回的是str类型的参数，可用getUrlParam拿到具体某个参数的值*/
  }
}
