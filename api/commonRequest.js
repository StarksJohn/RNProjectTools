...
export default async function ({ url, params = {}, method = 'GET', timeoutMs, text = false, promptForCustomToast = false, header, token }) {

  // Add the public parameters to be passed in each api
  const initCommonParams = method === 'GET' ? {
  ...
  const [err, res] = await tool.to(HttpManager.netFetch({
    url: url,
    params: /* (params instanceof Array) ? params : */ { ...params, ...initCommonParams },
    method,
    timeoutMs,
    text,
    header
  ...
  if (res) { // success
    console.log('commonRequest success', ' \n [Full url]', res.totalUrl, ' \n [header]', res.header, ' \n [url]', url, '\n [params]', params, ' \n' +
      ' [res]', res, ' \n [err]', err)

    return Promise.resolve(res.responseJson)
  } else { // fail
    console.log('commonRequest fail', ' \n [Full url]', err.totalUrl, ' \n [header]', err.header, ' \n [url]', url, '\n [params]', params, ' \n' +
      ' [err]', err)
    ...
    if (err instanceof Error) {
      ...
      return Promise.reject(message)
    } else if (code === netwrokCode.USER_BANNED || code === netwrokCode.TOKEN_EXPIRED || code === netwrokCode.Unauthorized) {
