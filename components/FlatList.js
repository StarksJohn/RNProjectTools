...
export default class BaseFlatList extends PureComponent {
  static propTypes = {
    name: PropTypes.string, pagingEnabled: PropTypes.bool, renderScrollTopBt: PropTypes.bool,/*是否画可点击后 滚回顶部的按钮*/
    renderRow: PropTypes.func, renderFooter: PropTypes.bool, showsVerticalScrollIndicator: PropTypes.bool,
    refresh: PropTypes.func, onMomentumScrollEnd: PropTypes.func,
    loadMore: PropTypes.func, showReFreshControl: PropTypes.bool,
    getItemLayout: PropTypes.func/*安卓必填可优化列表滑动,但ios 会造成 如首页 列表翻页时 cell 画不出来。妈的*/, componentDidUpdate: PropTypes.func,
    style: ViewPropTypes.style, renderEmptyList: PropTypes.func,
    refreshControlStyle: PropTypes.object,
    showScrollToTop: PropTypes.bool,/*右下角是否显示 向上箭头，点击后 列表直接 滚动到  顶部*/
    onFooterRefresh: PropTypes.func, onRefreshed: PropTypes.func,
    ...
    keyExtractor: PropTypes.func, footerTextStyle: ViewPropTypes.style,
    listState: PropTypes.func, onScroll: PropTypes.func,
    ...
    page: PropTypes.number, showListEmptyComponent: PropTypes.bool/*是否在列表没数据时 渲染组件*/,
  }

  static defaultProps = {
    name: '', style: { backgroundColor: '#F5F5F5' }, renderScrollTopBt: true, page: 0,
    renderRow: (item, index) => {
      return <View style={{ width: '100%', height: 100, }}>
      </View>
    }, showReFreshControl: true, cellHeight: 0,
    refresh: (name) => {
    },
    loadMore: (page, name) => {
    }, renderFooter: true, showsVerticalScrollIndicator: true,
    refreshControlStyle: _refreshControlStyle, showListEmptyComponent: true,
    showScrollToTop: true, onResetPressInEmptyList: null,
    renderEmptyList: null,
    footerRefreshingText: '数据加载中…',
    footerFailureText: '点击重新加载',
    footerNoMoreDataText: '别再划了，我也是有底线的~',
    footerEmptyDataText: '暂时没有相关数据',
    initialNumToRender: constant.listPageSize / 2,
    pageSize: constant.listPageSize, registTabBarOnTwiceClickEvent: false,
    ...
    listState: (data, state, pageSize) => {
      ...
      console.log('BaseFlatList listState=', listState)
      return listState
    },
  }
  ...
  constructor (props) {
    ...
    this.state = {
      ...
      listState: RefreshState.Idle, scrollEnabled: true,
      listHeight: 0,//为了 ListEmptyComponent
      page: this.props.page, registTabBarOnTwiceClickEvent: this.props.registTabBarOnTwiceClickEvent
    }
  ...
  refresh = () => {
    ...
    if (this.shouldStartHeaderRefreshing()) {
      if (!this.props.hasOwnProperty('refresh')) {
        console.log('BaseFlatList 的 refresh 属性没有')
        return
      }
      console.log('\nBaseFlatList 准备获取', this.props.name, ' 列表的 第 1 页数据')
      ...
      this.setState({
        listState: RefreshState.HeaderRefreshing, page: 1, data: []
      }, async () => {

        const [err, data] = await tool.to(refresh(this))
        if (data) {
          if (data && data.length > 0) {
            console.log('BaseFlatList 获取 ', name, ' 列表的 第', this.state.page, '页数据 成功，准备刷新ui，data=', data)
            this.setState({
              data: data, listState: this.changeListState(data), page: this.state.page, scrollEnabled: true
            }, () => {
              console.log('BaseFlatList.js 列表刷新 状态完毕')
              onRefreshed && onRefreshed(this)
            })
          } else {
            this.setState({
              data: [], listState: this.changeListState([]), page: 1, scrollEnabled: false,
            }, () => {
            })
          }
        } else {
          console.log('BaseFlatList 获取 ', name, ' 列表的 第', this.state.page, '页数据 失败')
          this.setState({
            data: [], listState: this.changeListState([]), page: 1, scrollEnabled: false,
          }, () => {
          })
  ...
  onEndReached = (info) => {
    ...
    if (this.shouldStartFooterRefreshing()) {
      ...
      this.setState({
        listState: RefreshState.FooterRefreshing, page: page + 1
      }, () => {
        console.log('\n BaseFlatList 准备获取', this.props.name, ' 列表的 第 ', this.state.page, '页数据')
        this.props.loadMore(this.state.page, this).then(
          (data) => {
            if (data && data.length > 0) {
              console.log('BaseFlatList 获取 ', this.props.name, ' 列表的 第', this.state.page, '页数据 成功，准备刷新ui，data=', data)
              this.setState({
                data: this.state.data.concat(data), listState: this.changeListState(data),
              }, () => {
                // this.page++
              })
            } else {
              this.setState({
                listState: this.changeListState([]), page: this.state.page - 1
              }, () => {
              })
            }
          }
          ...
          (e) => {
            console.log('BaseFlatList 获取 ', this.props.name, ' 列表的 第', this.state.page, '页数据 失败', e)
            this.setState({
              listState: this.changeListState([]), page: this.state.page - 1
            }, () => {
            })
  ...
  scrollToTop (animated = false) {
    ...
    if (this.state.data.length > 0) {
      if (Platform.OS === 'ios') {
        ...
        //ios 也没效果
        this.listRef && this.listRef.scrollToIndex({ viewPosition: 0, animated: true, index: 0, viewOffset: this.viewOffset })//
      }
    }
  }
   ...
   * @param animated
   */
  scrollToIndex (animated = false, cellH, index) {
    console.log('BaseFlatList scrollToIndex index=', index, ' cellH=', cellH)
    if (this.state.data.length > 0) {
      if (Platform.OS === 'ios') {
        this.listRef && this.listRef.setNativeProps({ contentOffset: { x: 0, y: index * cellH } })//安卓没效果
        // this.listRef && this.listRef.scrollToOffset({ offset: index * cellH, animated: false })
      } else if (this.state.data) {
        //ios 也没效果
        // this.listRef && this.listRef.scrollToIndex({ viewPosition: 0, animated, index })//
        this.listRef && this.listRef.scrollToOffset({ offset: index * cellH, animated: true })
      }
    }
  }

  renderScrollTopBt () {
    return this.props.renderScrollTopBt && <BasePureComponent
        ...
        (self) => {
          return self.state.show && this.props.showScrollToTop && (
            <TouchableOpacity
              ...
              onPress={() => {
                // this.SectionListR.scrollToLocation({ itemIndex: 0, animated: true }) 因 scrollToLocation 只能回滚到 sectionHeader，滚不到 ListHeader，故用下边的 直接 把 列表 位置 重置
                this.scrollToTop(true)
    ...
    switch (this.state.listState) {
          ...
          >
            {footerFailureComponent ? footerFailureComponent : (
              <View style={styles.footerContainer}>
                <Text style={[styles.footerText, footerTextStyle]}>{footerFailureText}</Text>
              </View>
            )}
          </TouchableOpacity>
        )
      ...
      case RefreshState.NoMoreData: {
        footer = footerNoMoreDataComponent ? footerNoMoreDataComponent : (
          <View style={styles.footerContainer}>
            <Text style={[styles.footerText, footerTextStyle]}>{footerNoMoreDataText}</Text>
          </View>
        )
        break
      }
  ...
  _renderEmpty () {
    ...
    return (!this.props.hasOwnProperty('renderHeader') && this.props.renderEmptyList) && (this.state.listState === RefreshState.Idle || this.state.listState === RefreshState.EmptyData) /*避免在下拉刷新时显示*/ ?
      <View style={{
        flex: 1,// backgroundColor: gRandomColor(),
        height: this.state.listHeight
      }}>
        {this.props.renderEmptyList(!this.props.onResetPressInEmptyList ? this.refresh : this.onResetPressInEmptyList)}
      </View> : <View />
  }

  componentDidUpdate (prevProps, prevState, snapshot) {
    console.log('BaseFlatList ', this.props.name, ' 列表 已重绘,state= ', this.state)
    this.props.componentDidUpdate && this.props.componentDidUpdate(prevProps, prevState, snapshot, this.state)
  }
  ...
  render () {
    const {
      name, renderRow, cellHeight, getItemLayout, style, refreshControlStyle, renderEmptyList, renderItem, initialNumToRender, keyExtractor, onScroll, ListHeaderComponent, pagingEnabled, renderScrollTopBt, renderFooter, showsVerticalScrollIndicator, onMomentumScrollEnd, showReFreshControl, showListEmptyComponent, ...rest
    } = this.props
    const { listHeight, data, listState, scrollEnabled, } = this.state
    console.log('BaseFlatList ', name, ' 列表 开始重绘，state=', this.state)
    // console.log('getItemLayout', getItemLayout)
    const _rest = {}
    let _getItemLayout = getItemLayout
    if (!_getItemLayout && cellHeight !== 0) {//没传 getItemLayout 但传了 cellHeight
      _getItemLayout = (data, index) => {
        return { length: cellHeight, offset: cellHeight * index, index }
      }
      _rest.getItemLayout = _getItemLayout
    }
    console.log('BaseFlatList _rest=', _rest)
    ...
    return <View style={[styles.container, style]}>
      <FlatList
        ...
        onLayout={(e) => {
          if (listHeight === 0 && e.nativeEvent.layout.height !== 0 && showListEmptyComponent) {
            console.log('BaseFlatList 设置 ' + name + ' 列表的 listHeight，马上重绘')
          ...
          ({ item, index }) => {
            if (item.id === -111) {
              return <View style={{
                flex: 1,
                width: '100%',
                height: gScaleSize(gStyles.appStyle.height),
        ...
        onMomentumScrollEnd={
          (e) => {
            console.log('BaseFlatList ', name, '列表 onMomentumScrollEnd e.nativeEvent=', e.nativeEvent)
        ...
        windowSize={gConstant.listPageSize}
        maxToRenderPerBatch={gConstant.listPageSize * 3} // 增量渲染最大数量
        updateCellsBatchingPeriod={50} // 增量渲染时间间隔
        // 限制 滚动速度,越小越慢
        decelerationRate={Platform.OS === 'android' ? 0.85 : 0.90}
