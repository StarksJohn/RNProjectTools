...
export default class BaseFlatList extends PureComponent {
  static propTypes = {
    ...
    getItemLayout: PropTypes.func/* 安卓必填可优化列表滑动,但ios 会造成 如首页 列表翻页时 cell 画不出来。妈的 */,
   ...
   * @param animated
   */
  scrollToIndex ({ animated = true, cellH, index = 0, viewPosition = 0, viewOffset = 0 }) {
    console.log('BaseFlatList scrollToIndex index=', index, ' cellH=', cellH)
    this.state.data.length > 0 && this.listRef?.scrollToIndex({ viewPosition: viewPosition, animated: animated, index: index, viewOffset: viewOffset })
  }
  ...
  _renderEmpty () {
    // console.log('BaseFlatList.js _renderEmpty listState=', this.state.listState)
    return (!this.props.hasOwnProperty('renderHeader') && this.props.renderEmptyList) && (this.state.listState === RefreshState.Idle || this.state.listState === RefreshState.EmptyData)
      ? /* 避免在下拉刷新时显示 */ <View style={{
      ...
      <FlatList
        ...
        onLayout={(e) => {
          if (listHeight === 0 && e.nativeEvent.layout.height !== 0 && showListEmptyComponent) {
            console.log('BaseFlatList 设置 ' + name + ' 列表的 listHeight，马上重绘,e.nativeEvent.layout.height=', e.nativeEvent.layout.height)
